...
void drawAxes(double x_g_min, double x_g_max, double y_g_min, double y_g_max,
    ...
    for (double y = y_shag; k_y * (y - y_max) > y_g_min + 10; y += y_shag) {
    ...
    for (double y = -y_shag; k_y * (y - y_max) < y_g_max; y -= y_shag) {
        axe_path.moveTo(ord_x - 5, k_y * (y - y_max));
        axe_path.lineTo(ord_x + 5, k_y * (y - y_max));
        axe_path.addText(ord_x - 20, k_y * (y - y_max) + 3, font, QString::number(y));
    }
    ...
    for (double x = x_shag; k_x * (x - x_min) < x_g_max - 10; x += x_shag) {
    ...
    for (double x = -x_shag; k_x * (x - x_min) > 0; x -= x_shag) {
        axe_path.moveTo(k_x * (x - x_min), abs_y - 5);
        axe_path.lineTo(k_x * (x - x_min), abs_y + 5);
        axe_path.addText(k_x * (x - x_min) - 3, abs_y + 20, font, QString::number(x));
    }
